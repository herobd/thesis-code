...
for i in range(1,len(sys.argv)):
    ...
    with open(csvfile, 'rb') as content:
        ...
        if manualEnd != blankEnd:
            outRow.append('nonstandard ending')
        elif len(data)>1:
            ...
            for index in range(1,len(data)+endIndex+1):
                transSent += int(data[endIndex][9])
                trueDone += int(data[endIndex][12])
                falseDone += int(data[endIndex][13])

            compsec = endCompletion/(endTime-startTime)
